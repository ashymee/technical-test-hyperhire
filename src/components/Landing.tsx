...
export default function Landing() {
  ...
  return (
    <div className="w-full h-full flex flex-col items-start justify-center gap-y-6 2xl:gap-y-8">
      <div className="h-52 md:h-[160] w-full relative overflow-hidden">
        <motion.div
          className="bg-[#8BC4FF] md:bg-white text-white md:text-[#40E2E8] font-poppins relative text-2xl font-extrabold rounded-lg px-3 py-[6] w-max before:block before:content-[''] before:absolute before:w-2 before:h-2 before:rotate-45 before:top-10 before:bg-[#8BC4FF] md:before:bg-white"
          initial={{ opacity: 0 }}
      ...
      <div className="hidden md:flex h-[70] relative w-full overflow-hidden">
        <motion.div
          className="w-full h-[70] absolute left-0 gap-y-4 flex flex-col overflow-hidden"
          initial={{ opacity: 0, top: 100 }}
          animate={{
            opacity: 1,
            delay: 50,
          ...
          <div className="font-poppins text-white flex text-[18]">{landingContent?.question}</div>
        </motion.div>
      </div>

      <div className="font-poppins text-white flex md:hidden text-[18]">{landingContent?.desc}</div>
      ...
      <div className="font-poppins text-white hidden md:flex gap-x-4 2xl:gap-x-8">
        {landingContent?.offers.map((item, index) => (
          <motion.div
            ...
            <p className="text-[16] 2xl:text-[18]">{item.name}</p>
...
export const LandingMobile = () => {
  ...
  return (
    <div className="flex flex-col gap-y-4 md:hidden font-poppins text-[18] w-full">
